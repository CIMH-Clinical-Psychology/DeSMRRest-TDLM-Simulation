...
n_segments = 8
n_shuf = 10  # for faster processing, replace later with real value
sim_sf_seg = np.full([len(subjects_incl), len(densities), n_segments, n_shuf, max_lag+1], np.nan)

tqdm_loop = tqdm(total=len(subjects_incl)*len(densities)*n_segments, desc='calculating simulation segments')

for s, subj in enumerate(tqdm(subjects_incl, desc="subject")):
    # train our classifier using localizer data and negative data
    train_x, train_y = localizer[subj]
    clf.fit(train_x[:, :, tp], train_y, neg_x=neg_x[subj], neg_x_ratio=2.0)
    for d, density in enumerate(densities):
        # there are 8 segments that are approximately 30 seconds long
        rs_sim = rs_sims[s, d, :, :]
        segments_sim = np.split(rs_sim, n_segments)

        for seg, segment_sim in enumerate(segments_sim):
            # get probability estimates for item reactivation from the resting state
            proba_sim_seg = clf.predict_proba(segment_sim)
            # calculate sequenceness
            sim_sf_subj, sim_sb_subj = tdlm.compute_1step(proba_sim_seg,
                                                          tf=tf,
                                                          n_shuf=n_shuf,
                                                          max_lag=max_lag,
                                                          alpha_freq=alpha_freq,
            ...
            sim_sf_seg[s, d, seg, :] = zscore_multiaxis(sim_sf_subj, axes=zscore_axes)
            tqdm_loop.update()

# determine signifiance thresholds per density
vmins = []
vmaxs = []
for d, dens in enumerate(densities):
    vmins += [np.min(np.mean(sx[:, d, :, 1:], 0))]
    vmaxs += [np.max(np.mean(sx[:, d, :, 1:], 0))]

# plot segments x densities
fig, ax = plt.subplots(1, 1, figsize=[10, 8])
sx = sim_sf_seg[:, :, :, :, lag_sim]
heatmap = np.nanmean(sx[..., 0], 0)
im = ax.imshow(heatmap.T, aspect='auto', cmap='PiYG')
ax.set_xlabel('replay density min-1')
ax.set_xticks(np.arange(0, len(densities))[::2], labels=densities[::2])
ax.set_ylabel('min. in resting state')
ax.set_yticks(np.arange(8))
ax.set_title(f'Sequenceness @ 70 ms across densities and blocks')
cbar = fig.colorbar(im)
cbar.set_label('forward sequenceness at 70 ms lag')

mask = np.zeros_like(heatmap, dtype=bool)
for d, density in enumerate(densities):
    thresh = vmaxs[d]
    mask[d][heatmap[d]>thresh] = True
...
#%% some normalization tryout
perf_test = [get_performance(subj=subj, which="test") for subj in subjects_incl]
colors_perf = [(0, 0, perf**3, perf**3) for perf in perf_test]
...
for d in [0, -1]:
    ...
    j=0
    ax = axs[j, d]; j+=1
    for i, sf in enumerate(sf_sim):
        ax.plot(sf[0], color=colors_perf[i])
    ax.set_title(f'{["baseline", "simulated replay"][d]}\nraw')

    ax = axs[j, d]; j+=1
    zscore_axes = [-1]
    for i, sf in enumerate(sf_sim):
        ax.plot(zscore_multiaxis(sf, axes=zscore_axes)[0], color=colors_perf[i])
    ax.set_title('zscored axis -1')

    ax = axs[j, d]; j+=1
    zscore_axes = [-1, -2]
    for i, sf in enumerate(sf_sim):
        ax.plot(zscore_multiaxis(sf, axes=zscore_axes)[0], color=colors_perf[i])
    ax.set_title('zscore axis [-2, -1]')

    ax = axs[j, d]; j+=1
    for i, sf in enumerate(sf_sim):
        auc = np.nansum(np.abs(sf[0]))
        ax.plot(sf[0]/auc, color=colors_perf[i])
    ax.set_title('normalize by AUC')

utils.normalize_lims(axs[0,:])
utils.normalize_lims(axs[1,:])
utils.normalize_lims(axs[2,:])
utils.normalize_lims(axs[3,:])
...
#%% Simulation RS with different oscillations

def generate_sine_wave(frequency, sample_rate, num_samples):
    """
    Generate a sinusoidal wave centered at 0.

    Parameters:
    frequency (float): The frequency of the sinusoid in Hz.
    sample_rate (float): The number of samples per second.
    num_samples (int): The total number of samples in the output wave.

    Returns:
    numpy.ndarray: The generated sinusoidal wave.
    """
...
def power_spectrum(signal, ufreq=np.inf, ax=None):
    ps = np.abs(np.fft.rfft(signal, axis=0))**2
    time_step = 1 / 100
    freqs = np.fft.rfftfreq(signal.shape[0], time_step)
    if ax is None:
        plt.figure()
        plt.plot(freqs[freqs<ufreq][1:], ps[freqs<ufreq][1:])
        ax = plt.gca()
    else:
        ax.plot(freqs[freqs<ufreq][1:], ps[freqs<ufreq][1:])
...
pkl_rs = settings.results_dir + '/pkl/simulation.pkl.gz'
...
# here we define replay densities that we want to simulate
# replay density is defined in events/minute or events^-1min
amplitudes = np.linspace(0, 0.1, 15).round(3)
n_steps = 1
...
# neg_x_ratio = 2.0
    # create axes to plot subject sequenceness into
# fig_sim, axs_sim = utils.make_fig(n_axs=len(subjects), bottom_plots=0, suptitle='Simulation results')

# convert sequence to ints
sequence = tdlm.utils.char2num(settings.seq_12)[:-1]

# save result in this dictionary
rs_sim_sf = np.full([len(subjects_incl), len(amplitudes), n_shuf, max_lag+1], np.nan)
rs_sim_sb = np.full([len(subjects_incl), len(amplitudes), n_shuf, max_lag+1], np.nan)
...
for s, subj in enumerate(tqdm(subjects_incl, desc="subject")):
    ...
    sine = generate_sine_wave(hz, sfreq, len(rs_pre))

    # insert into resting state data
    for a, amplitude in enumerate(tqdm(amplitudes, desc='amplitudes')):

        # add sine with specific amplitude
        rs_sim = (rs_pre.T + sine*amplitude).T

        plt.plot(rs_pre[:500, 0])
        plt.plot(rs_sim[:500, 0])

        # get probability estimates of resting state data for our classes
        proba_sim = clf.predict_proba(rs_sim)
        proba_sim2 = clf.predict_proba(rs_pre)

        fig, axs = plt.subplots(2, 1)
        power_spectrum(proba_sim2, 15, ax=axs[0])
        power_spectrum(proba_sim, 15, ax=axs[1])

        # calculate sequenceness
        sf_sim_subj, sb_sim_subj = tdlm.compute_1step(proba_sim, tf=tf, n_shuf=n_shuf,
                                                      max_lag=max_lag, alpha_freq=None)

        rs_sim_sf[s, a, :, :] = sf_sim_subj
        rs_sim_sb[s, a, :, :] = sb_sim_subj


# first plot all individual plots
...
for a, amplitude in enumerate(amplitudes):
    ...
    # for ax in axs.values(): ax.clear()
    ax.clear()
    fig.suptitle(f'Simulated replay amplitude {amplitude} ')
    tdlm.plot_sequenceness(sf_sim, sb_sim, which=['bkw', 'fwd'], title='forward sequenceness', ax=ax, rescale=False)
    # tdlm.plot_sequenceness(sf_sim, sb_sim, which=['fwd'], title='forward sequenceness', ax=axs['X'], rescale=False)
    # tdlm.plot_sequenceness(sf_sim, sb_sim, which=['bkw'], title='backward sequenceness', ax=axs['Y'], rescale=False)
    # axs['X'].legend(['forward sequenceness', 'max. perm. thresh.', '_', '95% perm. thresh.'])
    # axs['Y'].legend(['backward sequenceness', 'max. perm. thresh.', '_', '95% perm. thresh.'])
#     tdlm.plot_permutation_distribution(sf_sim, color=c_fwd, title="permutation distribution", ax=axs['A'])
#     tdlm.plot_permutation_distribution(sb_sim, color=c_bkw, title="perm. distr.", ax=axs['C'])
#     sf_sim_dict = dict(zip(subjects_incl, sf_sim[:, 0, 1:].mean(-1), strict=True))
#     sb_sim_dict = dict(zip(subjects_incl, sb_sim[:, 0, 1:].mean(-1), strict=True))
# #
#     r1, pval1 = plot_correlation(sf_sim_dict, values=perf_test, color=c_fwd, title="performance correlation", ax=axs['B'])
#     r2, pval2 = plot_correlation(sb_sim_dict, values=perf_test, color=c_bkw, title="performance correlation", ax=axs['D'])
#     utils.normalize_lims([axs['X'], axs['Y']])
#     utils.normalize_lims([axs['A'], axs['C']])
#     utils.normalize_lims([axs['B'], axs['D']])

    plt.pause(0.1)
    fig.tight_layout()
    # axs['B'].text(-0.001, 1.05, f'r={r1:.2f} p={pval1:.3f}')
    # axs['D'].text(-0.001, 1.05, f'r={r2:.2f} p={pval2:.3f}')
    plt.pause(0.1)
    fig.savefig(f'{settings.plot_dir}/simulation_oscillations-{amplitude:.3f}_step{n_steps}.png')
...
vmins = [np.min(np.mean(sx[:,:,1:,1:],0)) for sx in [rs_sim_sf, rs_sim_sb]]
vmaxs = [np.max(np.mean(sx[:,:,1:,1:],0)) for sx in [rs_sim_sf, rs_sim_sb]]

for i, sx in enumerate([rs_sim_sf, rs_sim_sb]):
    direction = ['fwd', 'bkw'][i]
    heatmap = np.mean(sx[:, :, 0, 1:], 0)
    ax = axs[i]
    im = ax.imshow(heatmap, aspect='auto', cmap='PiYG', vmin=min(vmins), vmax=max(vmaxs))
    ax.set_xlabel('time lag')
    ax.set_xticks(np.arange(0, 31, 5), np.arange(0, 320, 50))
    ax.set_ylabel(f'Simulated base amplitude {hz} Hz ')
    ax.set_yticks(np.arange(len(amplitudes)), np.round(amplitudes, 3))
    ax.set_title(f"{direction} sequenceness {hz}")
    plt.colorbar(im)
    plt.draw()
    plt.tight_layout()
fig.savefig(f'{settings.plot_dir}/simulation-oscillations-heatmap.png')
...
# # draw peak line graph
# fig, ax = plt.subplots(1, 1, figsize=[8, 6])
# threshholds = np.max(abs(np.mean(rs_sim_sf[:, :, 1:, 1:], 0)), -1)
# thresholds_95 = np.quantile(threshholds, 0.95, axis=-1)
# thresholds_max = np.nanmax(threshholds, axis=-1)
# peaks = np.nanmax(np.mean(rs_sim_sf[:, :, 0, 1:], 0), -1)

# ax.plot(peaks, label='peak sequenceness')
# ax.plot(thresholds_max, linestyle='-', color='gray', label='significance threshold')
# ax.plot(thresholds_95, linestyle='--', color='gray', label='significance threshold 95%')
# ax.set_yticks(np.arange(len(amplitudes)), amplitudes)
# ax.set_xlabel('7hz amplitde')
# ax.legend(loc='lower right')
# plt.pause(0.1)
# fig.tight_layout()
# plt.pause(0.1)
# fig.savefig(f'{settings.plot_dir}/simulation-{n_steps=}.png')


#%% Simulation RS with travelling waves
...
# amplitudes = np.linspace(0, 0.1, 15).round(3)
# amplitudes = [0.1]
speeds = np.arange(0, 100, 10)

layout = mne.channels.read_layout("Vectorview-all", scale=True)
...
# save result in this dictionary
rs_sim_sf = np.full([len(subjects_incl), len(speeds), n_shuf, max_lag+1], np.nan)
rs_sim_sb = np.full([len(subjects_incl), len(speeds), n_shuf, max_lag+1], np.nan)

montage = mne.channels.read_dig_fif('vectorview.fif')
chs_pos = [pos['r'][:2]*100 for pos in montage.dig]
fig, ax = plt.subplots(1, 1, figsize=[8, 5])
...
for i, subj in enumerate(tqdm(subjects_incl, desc="subject")):
    ...
    # insert into resting state data
    # for a, amplitude in enumerate(tqdm(amplitudes, desc='amplitudes')):
    for s, speed in enumerate(tqdm(speeds)):
        wave = tdlm.utils.create_travelling_wave(hz, sfreq, len(rs_pre), chs_pos, speed=speed)

        # add sine with specific amplitude
        rs_sim = rs_pre + wave*0.1

        # plt.plot(rs_pre[:500, 200])
        # plt.plot(rs_sim[:500, 200])
        ...
        # calculate sequenceness
        sf_sim_subj, sb_sim_subj = tdlm.compute_1step(proba_sim, tf=tf, n_shuf=n_shuf,
                                                      max_lag=max_lag, alpha_freq=alpha_control)

        rs_sim_sf[i, s, :, :] = sf_sim_subj
        rs_sim_sb[i, s, :, :] = sb_sim_subj
...
for s, speed in enumerate(speeds):
    ...
    # for ax in axs.values(): ax.clear()
    ax.clear()
    fig.suptitle(f'Simulated replay with speed {speed} {alpha_control=}')
    tdlm.plot_sequenceness(sf_sim, sb_sim, which=['bkw', 'fwd'], title='forward sequenceness', ax=ax, rescale=False)
    # tdlm.plot_sequenceness(sf_sim, sb_sim, which=['fwd'], title='forward sequenceness', ax=axs['X'], rescale=False)
    # tdlm.plot_sequenceness(sf_sim, sb_sim, which=['bkw'], title='backward sequenceness', ax=axs['Y'], rescale=False)
    # axs['X'].legend(['forward sequenceness', 'max. perm. thresh.', '_', '95% perm. thresh.'])
...
#     utils.normalize_lims([axs['B'], axs['D']])
    ...
    # axs['D'].text(-0.001, 1.05, f'r={r2:.2f} p={pval2:.3f}')
    plt.pause(0.1)
    fig.savefig(f'{settings.plot_dir}/simulation_wave-{speed:.3f}_step{n_steps}alphcontr{alpha_control}.png')
...
for i, sx in enumerate([rs_sim_sf, rs_sim_sb]):
    ...
    ax.set_xlabel('time lag')
    ax.set_xticks(np.arange(0, 31, 5), np.arange(0, 320, 50))
    ax.set_ylabel(f'propagation speed in cm/s {hz} Hz')
    ax.set_yticks(np.arange(0, len(speeds), 5), np.round(speeds, 3)[::5])
    ax.set_title(f"{direction} sequenceness {hz} {alpha_control=}")
    plt.colorbar(im)
    plt.draw()
    plt.tight_layout()
fig.savefig(f'{settings.plot_dir}/simulation-wave-heatmap-alphacontrol{alpha_control}.png')
