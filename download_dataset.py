...
def download_dataset(verify_existing=False):
    ...
    # multipart dataset
    record1 = zen.find_record_by_doi('10.5281/zenodo.8001754').data
    record2 = zen.find_record_by_doi('10.5281/zenodo.15629081').data

    all_files = record1['files'] + record2['files']

    # check which files are already downloaded
    totalsize = 0
    for file_d in tqdm(all_files, desc='verifying existing files'):
    ...
    loop = tqdm(total=len(all_files), desc='Downloading files', unit='MB')
    for file_d in all_files:
        md5 = file_d['checksum'].replace('md5:', '')
        target_file = settings.data_dir + f'/{file_d["key"]}'
        if os.path.exists(target_file):
            loop.update()
        ...
        check_md5(target_file, md5)
        loop.update()
    print(f"Finished downloading {len(all_files)} files, {totalsize:.1f}MB")
